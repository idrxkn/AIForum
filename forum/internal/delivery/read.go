...
func Read(w http.ResponseWriter, r *http.Request) {
	hystrix.ConfigureCommand("dbQuery", hystrix.CommandConfig{
		Timeout:               1000, // Timeout in milliseconds
		MaxConcurrentRequests: 100,  // Maximum number of concurrent requests allowed
		ErrorPercentThreshold: 50,   // Error percentage threshold to open the circuit
		SleepWindow:           5000, // Time in milliseconds to keep the circuit open before allowing retries
	})
	err := hystrix.Do("dbQuery", func() error {
...
func publishMessage(conn *amqp.Connection, message []byte) error {
